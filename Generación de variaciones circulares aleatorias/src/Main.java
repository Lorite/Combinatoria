...
public class Main {
	...
	*/
	...
	public static void inputUsuario() {
		...
		boolean datoValido = false;
		while (datoValido == false) {
			int numero = leerEnteroDeTeclado("Introduzca el número de elementos por conjunto: ");

			if (numero != -1 && numero <= m) {
				n = numero;
				break;
			}
			System.out.println("No has introducido un valor válido.");
	...
	public static int calcularVariacionesCirculares(int m, int n) {
		int total = 0;
		
		// fórmula
		total = factorial(m)/(n*factorial(m-n));
		
		return total;
	}
	
	public static ArrayList<ArrayList<String>> generarVariacionesCirculares(ArrayList<String> A, int n) {
...
//		}
	...
	public static void main(String[] args) {
		...
		int totalVariacionesOrdinarias = calcularVariacionesCirculares(m, n);
		...
		ArrayList<ArrayList<String>> conjuntoVariacionesOrdinarias = 
				generarVariacionesCirculares(A, n);
